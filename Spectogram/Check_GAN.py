...
Speech_gan = tf.saved_model.load('trained_wganlp_model_epoch_Dense_79_generator')
#print(gan.summary())
N_fft = 1024
...
for target_db in range (0,len(targets)):
 ...
 for No_of_data in range (0,No_of_data_to_filter):
   ...
   for No_of_overlaps in range (0,Overlaps-5): #need to fix this, ignores the last few parts
     ...
     Generated_speech_codebook_reshaped = np.abs((Generated_speech_codebook.reshape(1024,9)))
     ...
     scalar_factor_noise = sum([psd_value**2 for psd_value in GAN_noise_estimate]) #This code causes silence on some outputs. NVM I'm retarded. Try HIGHER SNR.
     scalar_factor_speech = sum([psd_value**2 for psd_value in GAN_speech_estimate])
     ...
     scalar_factor_noise = sum([psd_value**2 for psd_value in Estimated_noise_PSD])
     Estimated_noise_PSD_normalised = Estimated_noise_PSD/scalar_factor_noise

     Estimated_speech_PSD[512:1024]= np.flip(Estimated_speech_PSD[0:512])
     scalar_factor_speech = sum([psd_value**2 for psd_value in Estimated_speech_PSD])
     Estimated_speech_PSD_normalised = Estimated_speech_PSD/scalar_factor_speech

     
     PSD_of_real_windowed_speech_signal[0:512] = np.flip(PSD_of_real_windowed_speech_signal[0:512])
     scalar_factor_speech = sum([psd_value**2 for psd_value in PSD_of_real_windowed_speech_signal])
