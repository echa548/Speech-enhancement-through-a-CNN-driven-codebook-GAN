...
for No_of_data in range (0,len(directories)):
   myaudio = AudioSegment.from_file("creating_dataset2/dataset_all4/-6dB/"+ directories[No_of_data], "wav") 
   chunks = make_chunks(myaudio, chunk_length_ms)
   chosen_chunk = random.randint(0,len(chunks)-1)
   data = np.array(chunks[chosen_chunk].get_array_of_samples())
   data = np.transpose(data)
   #print(np.shape(data))
   Bit_Check = wave.open("creating_dataset2/dataset_all4/-6dB/"+ directories[No_of_data], 'rb')
   ...
   Mixture_PSD[No_of_data,:] = (np.abs(FFT_of_windowed_signal)**2)/(samplerate*PSD_window_scaling)
   myaudio=0
   chunks = 0
   myaudio = AudioSegment.from_file("creating_dataset2/dataset_all4/noise/-6dB/"+ directories2[No_of_data], "wav") 
   chunks = make_chunks(myaudio, chunk_length_ms)
   data = np.array(chunks[chosen_chunk].get_array_of_samples())
   #print(data)
   data = np.transpose(data)
   #print(np.shape(data))
   Bit_Check = wave.open("creating_dataset2/dataset_all4/noise/-6dB/"+ directories2[No_of_data], 'rb')
   bit_depth = Bit_Check.getsampwidth() * 8
   data = data/(2**(bit_depth-1))
   Rectangular_windowed_signal = data
   FFT_of_windowed_signal = Hann_window_a_signal(Rectangular_windowed_signal)
   Hann_window = sps.windows.hann(len(Rectangular_windowed_signal))
   PSD_window_scaling = np.sum(Hann_window**2)
   Clean_PSD[No_of_data,:] = (np.abs(FFT_of_windowed_signal)**2)/(samplerate*PSD_window_scaling)
   myaudio=0
   chunks = 0
