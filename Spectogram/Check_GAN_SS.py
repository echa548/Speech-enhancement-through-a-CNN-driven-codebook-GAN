...
for folder in range (0,len(targets)):
 directory = str(targets[folder])+'dB'
 path = os.path.join('Filter_outputs/Spectral_subtract',directory) 
 os.makedirs(path, exist_ok=True)
 ...
 for No_of_data in range (0,No_of_data_to_filter):
  ...
  #  real_speech = real_speech/(2**(bit_depth-1))
   ...
   for No_of_overlaps in range (0,Overlaps-5): #need to fix this, ignores the last few parts
     ...
     Rectangular_windowed_signal = Rectangular_windowed_signal[:,1]
     ...
     #TODO: This is a crude way of estimating the means for GMM derivation. i.e. make GMM just from means.
...
#     FFT_to_audio_perfect = np.fft.ifft(De_noised_frame_perfect)
     alpha = 0.02 #TODO: WORK WITH THIS

     #GAN Spectral subtraction

     snr = PSD_of_windowed_signal/(100*GAN_noise_estimate)
     GAN_spectral_mask = 1 - np.minimum(1, np.maximum(0, alpha * snr))
     GAN_clean = FFT_of_windowed_signal * GAN_spectral_mask
     GAN_frames = np.fft.ifft(GAN_clean)
     
     #Wiener filter Spectral subtraction
     snr_w = PSD_of_windowed_signal/Estimated_noise_PSD
     Wiener_spectral_mask = 1 - np.minimum(1, np.maximum(0, alpha * snr))
     Wiener_clean = FFT_of_windowed_signal * Wiener_spectral_mask
     Wiener_frames = np.fft.ifft(Wiener_clean)



     #audio[0+128*No_of_overlaps:512+128*No_of_overlaps] = audio[0+128*No_of_overlaps:512+128*No_of_overlaps]+FFT_to_audio[0:512] #THIS IS THE OLD WIENER FILTER
     audio[0+128*No_of_overlaps:512+128*No_of_overlaps] = audio[0+128*No_of_overlaps:512+128*No_of_overlaps]+Wiener_frames[0:512] #recover only the windowed signal and not the zero-pad
...
#     audio_GAN_Noise_only[0+128*No_of_overlaps:512+128*No_of_overlaps] = audio_GAN_Noise_only[0+128*No_of_overlaps:512+128*No_of_overlaps]+FFT_to_audio_GAN_noise_GMM_speech[0:512] #THIS IS GAN NOISE AND GMM SPEECH
#    audio_perfect[0+128*No_of_overlaps:512+128*No_of_overlaps] = audio_perfect[0+128*No_of_overlaps:512+128*No_of_overlaps]+FFT_to_audio_perfect[0:512] #recover only the windowed signal and not the zero-pad
     audio_ss[0+128*No_of_overlaps:512+128*No_of_overlaps] = audio_perfect[0+128*No_of_overlaps:512+128*No_of_overlaps]+GAN_frames[0:512]
  ...
  #  sf.write('Filter_outputs/Perfect/'+str(targets[target_db])+'dB/'+directories[No_of_data], audio_perfect, 16000, 'PCM_16')
   sf.write('Filter_outputs/Spectral_subtract/'+str(targets[target_db])+'dB/'+directories[No_of_data], audio_ss, 16000, 'PCM_16')
